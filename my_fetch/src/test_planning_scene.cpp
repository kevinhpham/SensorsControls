...
{
  ros::init(argc, argv, "fetch_arm_kinematics");
  ...
  moveit::core::RobotStatePtr current_state(new moveit::core::RobotState(kinematic_model));
...
// Getting Contact Information
  ...
  // check for directly.
    ...
    ROS_INFO_STREAM("Moving joint values to self intersect the arm");
  std::vector<double> joint_values = { 0, 1.57, 0.0,-1.2, 0.0, 1, 0.0 }; //sets arm to self intersect
  current_state->setJointGroupPositions(joint_model_group, joint_values);
  ROS_INFO_STREAM("Test 4: Current state is "<< (current_state->satisfiesBounds(joint_model_group) ? "valid,within joint limits" : "not valid,outside joint limits"));
...
// Constraint Checking
  ...
  desired_pose.header.frame_id = "wrist_roll_link";
